...
use crate::instruction::{get_flattened_instructions, WrappedInstruction};
...
impl<'a> TransactionContext<'a> {
    fn new(transaction: &'a ConfirmedTransaction) -> Self {
        ...
        .into_string();
        let num_required_signatures = transaction
            .transaction
            .as_ref()
            .unwrap()
            .message
    ...
    pub fn build(transaction: &'a ConfirmedTransaction) -> Result<Self, &'static str> {
        ...
        for (pre_balance, post_balance) in pre_balances
            .iter()
            .cloned()
            .zip(post_balances.iter().cloned())
        {
            context.account_balances.push(AccountBalance {
                pre_balance,
                post_balance,
            });
        }
    ...
    pub fn update_accounts(&mut self, instruction: &WrappedInstruction) {
        ...
        match TokenInstruction::unpack(&instruction.data()) {
            Ok(TokenInstruction::InitializeAccount) => {
                let token_account = parse_token_account_from_initialize_account_instruction(
                    instruction,
                    self,
                    None,
                );
        ...
        match TokenInstruction::unpack(&instruction.data()) {
            ...
            Ok(TokenInstruction::Transfer { amount }) => {
                ...
                let destination_account =
                    self.token_accounts.get_mut(&destination_address).unwrap();
                destination_account.post_balance =
                    destination_account.post_balance.map(|x| x + amount);
            }
...
fn parse_token_account_from_initialize_account_instruction<'a>(
    instruction: &WrappedInstruction,
    context: &TransactionContext<'a>,
    owner: Option<Pubkey>,
) -> TokenAccount<'a> {
    let address = context.accounts[instruction.accounts()[0] as usize].clone();
    let mint = context.accounts[instruction.accounts()[1] as usize]
        .to_pubkey()
        .unwrap();
    let owner = match owner {
        Some(pubkey) => pubkey,
        None => context.accounts[instruction.accounts()[2] as usize]
            .to_pubkey()
            .unwrap(),
    };
    let balance = if mint != WRAPPED_SOL_MINT {
        Some(0)
    } else {
        None
    };
...
) -> Result<TransactionContext<'a>, Error> {
...
pub fn get_signers(transaction: &ConfirmedTransaction) -> Vec<String> {
    ...
    let num_required_signatures = transaction
        ...
        .unwrap()
        .num_required_signatures;
    accounts[..num_required_signatures as usize]
        .iter()
        .map(|x| x.to_string())
        .collect()
}
