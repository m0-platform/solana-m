...
impl<'a> StructuredInstruction<'a> {
    fn new(
        ...
        let instruction_accounts: Vec<_> = instruction
            .accounts()
            .iter()
            .map(|i| PubkeyRef(accounts[*i as usize]))
            .collect();
    ...
    pub fn program_id(&self) -> PubkeyRef<'a> {
        self.program_id
    }
    pub fn program_id_index(&self) -> u32 {
        self.instruction.program_id_index()
    }
    pub fn accounts(&self) -> &Vec<PubkeyRef> {
        &self.accounts
    }
    pub fn data(&self) -> &Vec<u8> {
    ...
    pub fn inner_instructions(&self) -> Ref<Vec<Rc<Self>>> {
        self.inner_instructions.borrow()
    }
    pub fn parent_instruction(&self) -> Option<Rc<Self>> {
        self.parent_instruction
            .borrow()
            .as_ref()
            .map(|x| x.upgrade().unwrap())
    }
    pub fn logs(&self) -> Ref<Option<Vec<Log<'a>>>> {
        self.logs.borrow()
    }
    ...
    I: Iterator<Item = Log<'a>>,
{
    ...
    for instruction in flattened_instructions {
        let structured_instruction = Rc::new(StructuredInstruction::new(
            instruction,
            Vec::new().into(),
            &accounts,
        ));
        ...
        while !instruction_stack.is_empty()
            && instruction_stack.last().unwrap().stack_height()
                >= structured_instruction.stack_height()
        {
            let popped_instruction = instruction_stack.pop().unwrap();
            *popped_instruction.logs.borrow_mut() =
                log_stack.close(logs, popped_instruction.program_id());

            if !instruction_stack.is_empty() {
                *popped_instruction.parent_instruction.borrow_mut() =
...
pub fn get_flattened_instructions(
        ...
        if let Some(inner) = inner_instructions.get(j) {
            if inner.index == i as u32 {
                wrapped_instructions.extend(
                    inner_instructions[j]
                        .instructions
                        .iter()
                        .map(|x| WrappedInstruction::from(x)),
                );
                j += 1;
            }
        }
...
pub fn get_structured_instructions<'a>(
    transaction: &'a pb::ConfirmedTransaction,
) -> Result<Vec<Rc<StructuredInstruction<'a>>>, Error> {
    if let Some(_) = transaction.meta.as_ref().unwrap().err {
        return Err(anyhow!(
            "Cannot structure instructions of a failed transaction."
        ));
    }
    ...
    Ok(structure_flattened_instructions_with_logs(
        flattened_instructions,
        &mut logs.iter().map(|log| Log::new(log)).peekable(),
        accounts,
    ))
}
